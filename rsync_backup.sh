...
echo -e "${B}
  ...
  ██║  ██║███████║   ██║   ██║ ╚████║╚██████╗    ███████║███████╗██║  ██║ ╚████╔╝ ███████╗██║  ██║
  ╚═╝  ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═══╝ ╚═════╝    ╚══════╝╚══════╝╚═╝  ╚═╝  ╚═══╝  ╚══════╝╚═╝  ╚═╝
  Rsync Backup Script Server - version 1.0.1
${N}"
}
...
fn_log_info()  { printf '%s\n' "$APPNAME: $1"; }
fn_log_warn()  { printf '%s\n' "$APPNAME: [WARNING] %s" "$1" >&2; }
fn_log_error() { printf '%s\n' "$APPNAME: [ERROR] %s" "$1" >&2; }
...
fn_find() {
  ...
  local extra="$*"
  if [ -n "$extra" ]; then
    fn_run_cmd "find '$target' $extra"
  else
    fn_run_cmd "find '$target'"
  fi
}
